...
function App() {
  ...
  useEffect(() => {
      setResult1(text1.replace(/[^a-zA-Z]+/g, " ").split(" ").filter(word => word.match(/\bse/g)));
  }, []);
  return (
    <div className="App">
      <div className="box">
        <p>match all words that start with "se"</p>
        <p>text1.replace(/[^a-zA-Z]+/g, " ").split(" ").filter(word => word.match(/\bse/g))</p>
        <h3>Text</h3>
        <p>{text1}</p>
